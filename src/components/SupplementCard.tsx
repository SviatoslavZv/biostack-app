  ...
  return (
    ...
    >
      ...
      <div className="relative w-full h-40 mb-2.5 rounded-[1.5rem] bg-gray-50/50 group/img overflow-hidden border border-transparent transition-colors group-hover/main-card:border-green-100/30 group/card">
        <div className="absolute inset-0 z-10 pointer-events-none">
          <div className="relative w-full h-full p-2.5">
            ...
            {/* Передняя сторона (Floating effect: scale + shadow) */}
            <div className="absolute inset-0 p-2.5 transition-all duration-500 group-hover/card:opacity-0 group-hover/card:scale-110">
              <Image
                src={item.imageFront}
                alt={item.name}
                fill
                // Добавляем priority. Если true, то Next.js применит loading="eager" автоматически
                priority={isPriority}
            ...
            <div className="absolute inset-0 p-2.5 transition-all duration-700 opacity-0 group-hover/card:opacity-100 group-hover/card:scale-[2.8] origin-center">
              <Image
                ...
                sizes="(max-width: 768px) 100vw, 50vw" // Тут картинка крупнее из-за зума
              />
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col flex-1 text-center">
        ...
        {/* Цена и доза (Interactive highlight) */}
        <div className="flex items-baseline justify-center flex-wrap gap-x-8 mt-auto">
          ...
          <span className="text-[11px] font-bold text-slate-500/90 transition-colors duration-300 group-hover/main-card:text-green-700 truncate tracking-tight">
            ${(item.price / item.servings).toFixed(2)} / dose
          </span>
        </div>

        {/* Кнопка (Perfectly aligned) */}
        <div className="pt-3">
          {count > 0 ? (
